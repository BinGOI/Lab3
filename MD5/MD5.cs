    ...
    public class MD5
    {
        ...
        public static byte[] HexToBytes(string hexNumber)
        {
            if(hexNumber.Length == 8)
                return BitConverter.GetBytes(uint.Parse(hexNumber , System.Globalization.NumberStyles.HexNumber));

            if (hexNumber.Length == 16)
                return BitConverter.GetBytes(ulong.Parse(hexNumber, System.Globalization.NumberStyles.HexNumber));

            throw new Exception("MD5.HexToBytes is only for uint and ulong");
        }
    }
}
